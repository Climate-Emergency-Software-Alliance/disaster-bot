...
class WhatsappCloud {
        ...
        unirest(method, fullUrl)
          ...
          .end(function (res) {
            if (res.error) {
              let errorObject = () => {
                try {
                  console.log("Error while sending message", res.body);
                  return res.body?.error || JSON.parse(res.raw_body);
                } catch (e) {
                  console.log("Error in catch bl", e);
                  return {
                    error: res.raw_body,
                  };
                }
  ...
  async sendRadioButtons({
    ...
    let samples = {
      ...
      interactive: {
        ...
        action: {
          button: actionTitle || "Select a product",
  ...
  async sendTemplate({ template, message, recipientPhone, languageCode }) {
    // to do: context is not working
    this._mustHaverecipientPhone(recipientPhone);
    let body = {
      messaging_product: "whatsapp",
